...
STATIC inline int hash(const int variant , uint8_t* out, size_t outlen,
                       const uint8_t* in, size_t inlen,
                       size_t rate, uint8_t delim) {
  if ((out == NULL) || ((in == NULL) && inlen != 0) || (rate > Plen) || (outlen > Plen)) {
    return -1;
  }
  ...
  a[variant][inlen] ^= delim;
  a[variant][rate - 1] ^= 0x80;
  // Xor in the last block.
  xorin(a[variant], in, inlen);
  // Apply P
  P(a[variant]);
  // Squeeze output.
  for (size_t i = 0; i < outlen; i++) out[i] = a[variant][i];
  return 0;
}
...
kernel void heavy_hash(
    ...
    Hash hash_;
    hash(0, hash_, 32, (uint8_t *) buffer, 80, 136, 0x04);
    ...
    hash(1, hash_, 32, hash_, 32, 136, 0x04);
